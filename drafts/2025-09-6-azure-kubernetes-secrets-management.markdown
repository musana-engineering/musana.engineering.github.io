---
layout: post
title: Azure Kubernetes Secrets Management - The Right Way!
date: 2025-09-6 13:32:20 +0300
description: A practical guide to implementing Azure's native secrets management for AKS.
img: ml_cover.jpg # Add image post (optional)
fig-caption: # Add figcaption (optional)
tags: [aks, keyvault, kubernetes, secrets, csi, devops]
---

Managing secrets is one of the most fundamental yet persistent challenges in Kubernetes. The built-in Secret object, while convenient, was never designed to be a secure secret store. This forces platform engineers to devise their own strategies, each with its own trade-offs in complexity, security, and operational overhead.

If you're running on Azure Kubernetes Service (AKS), you've likely faced these core challenges:

- The Etcd Security Problem: Base64-encoded Kubernetes Secrets are not encrypted by default and are stored in etcd. Anyone with cluster admin access can read them.
- The Lifecycle Management Problem: Rotating a secret means updating the Kubernetes Secret, potentially redeploying workloads, and ensuring all pods get the new value. This process is manual and error-prone.
- The Centralization Problem: Secrets are often created and stored in a secure vault like Azure Key Vault, but getting them into Kubernetes requires a complex, custom pipeline.
- The Permissions Problem: Granting pods the right permissions to access Key Vault often involves managing service principals, client secrets, or complex pod identity configurations.

For years, the answers were complex tools like External Secrets Operator (ESO) or baking the Azure SDK directly into application code. But what if the platform itself could solve this for you?

Enter AKS Service Connector. It’s not just another tool; it’s a platform-level solution that directly addresses these challenges.
...
### Prerequisites
This is a hands-on technical implementation so before we dive in, ensure you have the following


AzureKeyvaultSecretProvider addon is enabled on your cluster, and its identity is assigned the keyvault secret access permission. Please check the sample code to create the SecretProviderClass and workload resources to access your keyvault.
